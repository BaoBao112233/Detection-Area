...
# Kiểm tra xem điểm có thuộc mặt phẳng không
...
class YoloDetect():
    ...
    # Chuyển tin nhắn lên telegram
    def alert(self, img):
        cv2.putText(img, "ALARM!!!!", (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        # New thread to send telegram after 15 seconds
        if (self.last_alert is None) or (
                (datetime.datetime.utcnow() - self.last_alert).total_seconds() > self.alert_telegram_each): # ktra và gửi tin nhắn sau mỗi 15s
    ...
    # Detect Yolo, chuyển ảnh thảnh blob để đưa vào model
